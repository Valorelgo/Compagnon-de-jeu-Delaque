            ...
            <div id="skills-list">
                ${m.skills.length === 0 ? '<p>Aucune compétence sélectionnée.</p>' : m.skills.map((s, i) => {
                    ...
                    <div style="margin:6px 0; background:#181818; padding:6px 8px; border-radius:4px; display:flex; justify-content:space-between; align-items:flex-start;">
                        ...
                        ${(!isPostCycleEquip && !isBeast && !isMerc && (!isPoisonBlood || m.charId !== 'char_death_maiden') && (!isLandsOnFeet || m.charId !== 'char_phyrr_cat') && (!isJuggernaut || !isBrute) && !isInnateLeader && !isInnateChampion) ? `<button class="btn-danger" style="padding:2px 6px; font-size:11px; flex-shrink:0;" onclick="removeSkill(${i})">X</button>` : ''}
                    </div>
                    `;
                }).join('')}
            </div>
