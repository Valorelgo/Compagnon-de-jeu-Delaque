...
const APP_GANG_FACTION = "Delaque";
...
function migrateSavedGangsIfNeeded() {
    const storedVersion = parseInt(localStorage.getItem('delaqueGangsSchemaVersion'), 10) || 0;
    if (storedVersion >= GANGS_SCHEMA_VERSION) return;

    if (storedVersion < 2) {
        // Ajout du tag de faction : toute sauvegarde déjà présente dans ce
        // programme Escher-only est réputée Escher si elle n'a pas encore de tag
    ...
    localStorage.setItem('delaqueGangsSchemaVersion', String(GANGS_SCHEMA_VERSION));
    saveGangs();
}
...
// (escherGangs_otherFactions) : aucune perte de données, la liste étrangère reste
// récupérable, mais n'apparaît plus jamais dans cette appli Escher.
function quarantineForeignFactionGangs() {
    ...
    let quarantine = {};
    try {
        quarantine = JSON.parse(localStorage.getItem('delaqueGangs_otherFactions')) || {};
    } catch (e) {
        quarantine = {};
    }
    ...
    localStorage.setItem('delaqueGangs_otherFactions', JSON.stringify(quarantine));
