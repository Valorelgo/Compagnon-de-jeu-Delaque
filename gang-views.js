...
function renderMenu(container) {
    ...
    updateTopBar();
    container.innerHTML = `
        <div class="card" style="text-align:center; border-color:var(--accent-cyan); box-shadow:0 0 25px rgba(45,212,191,0.15);">
            <h2 style="color:var(--accent-cyan); letter-spacing:2px; margin-bottom:2px;">COMPAGNON DE GANG DELAQUE</h2>
            <p style="color:var(--accent-purple); font-style:italic; margin-bottom:22px; font-size:13px;">« Dans l'ombre, on ne voit que ce qu'on choisit de montrer. »</p>
            <button onclick="navigate('gang-create')">Créer un nouveau gang</button><br>
            <button onclick="appState.mode='campaign'; navigate('gang-select')">Suivre un gang (Campagne)</button><br>
            <hr style="border-color:var(--border-color); margin: 20px 0;">
            <button onclick="importGang()">Importer un gang Delaque (.json)</button>
        </div>
    `;
}
...
function renderGangSelect(container) {
    ...
    let delaqueGangNames = Object.keys(savedGangs).filter(name => {
        const g = savedGangs[name];
        return !g.faction || g.faction === APP_GANG_FACTION;
    });
    if(delaqueGangNames.length === 0) {
        html += `<p>Aucun gang sauvegardé.</p>`;
    } else {
        for(let name of delaqueGangNames) {
...
function loadGang(name) {
    ...
    if (g.faction && g.faction !== APP_GANG_FACTION) {
        return showToast(`"${name}" est une liste ${g.faction}, pas Delaque. Ce programme ne peut pas l'ouvrir.`, "error");
    }
    currentGang = g;
